...
# Installation via un de mes scripts uinInstall de notre gitlab 
##cf: https://gitlab.com/mbomaerick401/uinstall
f_action_uinstall() {
	if [[ "$GUI" == *"$1"* ]]
	then
		wget https://gitlab.com/mbomaerick401/uinstall/-/tree/main_"$2" --no-check-certificate
		chmod +x uinstall_"$2" ; sudo ./uinstall_"$2" ; rm uinstall_"$2"
	fi
}
