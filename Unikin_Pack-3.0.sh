...
if [ "$UID" -eq "0" ]
then
    echo -e "${rouge}Merci de ne pas lancer directement ce script avec les droits root : lancez le sans sudo (./Unikin_Pack-3.0.sh), le mot de passe sera demandé dans le terminal lors de la 1ère action nécessitant le droit administrateur.${neutre}"
    exit
fi
...
then
	...
	f_action_ppa_install "$CA_CHROMIUMBETA" ppa:saiarcot895/chromium-beta "chromium-browser"
   	...
   	f_action_uinstall "$CA_FIREFOXDEVELOPER" FirefoxDeveloperEdition
	...
	f_action_flatpak_install "$CA_ONLYOFFICE" org.onlyoffice.desktopeditors
    	f_action_uinstall "$CA_OPENOFFICE" OpenOffice
	...
	f_action_snap_install "$CA_PROJECTLIBRE" "projectlibre" 
    	f_action_uinstall "$CA_SCENARI" Scenari
	...
	f_action_uinstall "$CA_MOLOTOVTV" Molotov ##(AppImage récupéré auto par le script)
	...
	f_action_uinstall "$CA_CELESTIA" Celestia ##(AppImage récupéré par le script)
	...
	f_action_uinstall "$CA_BITCOINCORE" BitcoinCore
	...
	f_action_uinstall "$CA_MULTISYSTEM" Multisystem
	f_action_uinstall "$CA_MYTOURBOOK" Mytourbook
	...
	f_action_uinstall "$CA_GEEKBENCH" Geekbench
	...
	f_action_exec "$CA_UT4" "wget https://gitlab.com/simbd/uinstall_Software/raw/master/uinstall_UnrealTournament4 && chmod +x LinI*Unreal*"
	...
	f_action_uinstall "$CA_GDEVELOP" Gdevelop
